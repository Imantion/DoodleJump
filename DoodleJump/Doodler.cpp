...
Projectile* Doodler::shoot(float shootPointX, float shootPointY, MemoryPool* projectilePool)
{
	...
	Projectile* projectile = static_cast<Projectile*>(projectilePool->allocate());

	if (projectile)
	{
		projectile->reset();
		projectile->setVelocity(velocityX, velocityY);
		projectile->setPosition(doodlerCenter, spriteRectangle.y);
	}
